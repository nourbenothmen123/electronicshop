...
<body class="font-opensans">
            ...
            <div id="page_top" class="section-body">
                <div class="container-fluid">
                    <div class="page-header">
                        <div class="left">
                            <h1 class="page-title">Liste des catégorie-attribut</h1>
                        </div>
                        @auth
                    ...
                    <div class="row clearfix">
                        <div class="col-lg-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <a href="{{ route('CategorieAttribute-form') }}" id="addToTable"
                                        class="btn btn-primary mb-15" type="button">
                                        <i class="icon wb-plus" aria-hidden="true"></i> Ajouter un attribut pour une
                                        catégorie
                                    </a>
                                    <div class="row">
                                        ...
                                        <form action="{{ route('Categorie-Attribute') }}" method="GET"
                                            class="col-sm-12 col-md-6">
                                            <div id="addrowExample_filter" class="dataTables_filter">
                                                <label>catégorie:<input type="search" class="form-control"
                                                        placeholder="" aria-controls="addrowExample"
                                                        name="searchCategory" value="{{ $searchCategory }}"></label>
                                            </div>
                                            <div id="addrowExample_filter" class="dataTables_filter">
                                                <label>attribute:<input type="search" class="form-control"
                                                        placeholder="" aria-controls="addrowExample"
                                                        name="searchAttribute" value="{{ $searchAttribute }}"></label>
                                            </div>
                                    ...
                                    @if ($categoryAttributes->count() > 0)
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <table class="table table-hover table-vcenter table-striped" cellspacing="0"
                                                ...
                                                <tbody>
                                                    @foreach ($categoryAttributes as $categorieAttribute)
                                                        <tr class="gradeA">
                                                            <td>{{ $categorieAttribute->id }}</td>

                                                            <td>
                                                                @if ($category = App\Models\Category::find($categorieAttribute->categorie_id))
                                                                    {{ $category->name }}
                                                                @else
                                                            ...
                                                            <td>
                                                                @if ($attribute = App\Models\Attribute::find($categorieAttribute->attribut_id))
                                                                    {{ $attribute->name }}
                                                                @else
                                                            ...
                                                            <td class="actions">
                                                                <a href="/modifier-CategorieAttribute/{{ $categorieAttribute->id }}"
                                                                ...
                                                                <button type="submit"
                                                                    ...
                                                                    data-toggle="tooltip"
                                                                    data-original-title="Supprimer" onclick="confirmDelete({{ $categorieAttribute->id }})">
                                                                    <i class="fa fa-trash fa-lg"
                                                                        aria-hidden="true"></i>
                                                                </button>
                                                                <form method="POST"
                                                                    action="{{ route('CategorieAttribute.destroy', $categorieAttribute->id) }}" id="delete-form-{{ $categorieAttribute->id }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                            </td>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 col-md-5">
                                            <div class="dataTables_info" id="addrowExample_info" role="status"
                                                aria-live="polite">Afficher de 1 à {{ $categoryAttributes->count() }}
                                                sur {{ $totalEntries }} attribut-catégorie</div>
                                        </div>
    ...
    <script>
        function confirmDelete(id) {
            Swal.fire({
                title: "Voulez-vous supprimer l'association entre l'attribut et la catégorie ?",
                text: " ",
                icon: 'warning',
