            ...
            <div id="page_top" class="section-body">
                <div class="container-fluid">
                    <div class="page-header">
                        <div class="left">
                            <h1 class="page-title">Liste des valeurs variation-attribut</h1>
                        </div>
                        @auth
                    ...
                    <div class="row clearfix">
                        <div class="col-lg-12">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <a href="{{ route('Ajouter-ValeurAttribute-form') }}" id="addToTable"
                                        class="btn btn-primary mb-15" type="button">
                                        <i class="icon wb-plus" aria-hidden="true"></i>Ajouter une valeur d'attribut au
                                        variation
                                    </a>
                                    <div class="row">
                                        ...
                                        <form action="{{ route('Valeur-Attribute') }}" method="GET"
                                            class="col-sm-12 col-md-6">
                                            <div id="addrowExample_filter" class="dataTables_filter">
                                                <label>Nom variation:<input type="search" class="form-control"
                                                        placeholder="" aria-controls="addrowExample"
                                                        name="searchVariation" value="{{ $searchVariation }}"></label>
                                            </div>
                                            <div id="addrowExample_filter" class="dataTables_filter">
                                                <label>Nom attribut:<input type="search" class="form-control"
                                                        placeholder="" aria-controls="addrowExample"
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <table class="table table-hover table-vcenter table-striped" cellspacing="0"
                                            ...
                                            <tbody>
                                                @foreach ($valeurAttributes as $valeurAttribute)
                                                    <tr class="gradeA">
                                                        <td>{{ $valeurAttribute->id }}</td>
                                                        <td>
                                                            @if ($variation = App\Models\Variation::find($valeurAttribute->variation_id))
                                                                {{ $variation->name }}
                                                            @else
                                                        ...
                                                        <td>
                                                            @if ($attribute = App\Models\Attribute::find($valeurAttribute->attribut_id))
                                                                {{ $attribute->name }}
                                                            @else
                                                                Aucun attribut associé
                                                            @endif
                                                        </td>
                                                        <td>{{ $valeurAttribute->valeur }}</td>
                                                        <td class="actions">
                                                           
                                                                <a href="{{ route('modifier-ValeurAttribute-form', ['id' => $valeurAttribute->id]) }}"
                                                                ...
                                                                <button type="submit"
                                                                    ...
                                                                    onclick="confirmDelete({{ $valeurAttribute->id }})">
                                                                    <i class="fa fa-trash fa-lg"
                                                                        aria-hidden="true"></i>
                                                                </button>
                                                                <form method="POST"
                                                                action="{{ route('ValeurAttribute.destroy', $valeurAttribute->id) }}" id="delete-form-{{ $valeurAttribute->id }}">
                                                                @csrf
                                                                @method('DELETE')
                                                            </form>
                                                        </td>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 col-md-5">
                                            <div class="dataTables_info" id="addrowExample_info" role="status"
                                                aria-live="polite">Afficher de 1 à {{ $valeurAttributes->count() }}
                                                sur {{ $totalEntries }} valeurs-attribut</div>
                                        </div>
