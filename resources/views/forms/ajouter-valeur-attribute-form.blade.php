            ...
            <div class="section-body">
                <div class="container-fluid">
                    <div class="row clearfix">
                        <div class="col-lg-12">
                            <form action="{{ route('ValeurAttribute.create') }}" method="POST">
                                @csrf
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Ajouter une valeur pour un attribut d'une variation</h3>
                                    </div>
                                    <div class="card-body">
                                        <input type="hidden" name="variation_id" value="{{ $variation->id }}">
                                        {{-- @dump($variation->id, 'VarId') --}}
                                        <input type="hidden" name="product_id" value="{{ $variation->product_id }}">

                                        {{-- @dump($variation->product_id, 'prodId') --}}
                                        @foreach ($categoryAttributes as $categoryAttribute)
                                            ...
                                            @error("attributes.$categoryAttribute->id.valeur")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        @endforeach
                                        <button type="submit" class="btn text-white bg-green">Enregistrer</button>
                                    </div>
